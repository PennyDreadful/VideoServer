...
public class AutoGradingTest {
	...
	@Rubric(
			value="Updating a video's rating returns the new average",
			goal="The server should store and update a rating for each video",
			points=10.0,
			reference="This test is my own"
			)
	@Test
	public void testAddRating() throws Exception {
		Video received = videoSvc.addVideo(video);
		long id = received.getId();
		double rating = videoSvc.addRating(id, 1);
		assertEquals(1.0, rating, 10e-6);
		rating = videoSvc.addRating(id, 5);
		assertEquals(3.0, rating, 10e-6);
